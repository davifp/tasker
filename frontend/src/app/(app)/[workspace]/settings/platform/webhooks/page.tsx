...
import { WebhooksView } from '@/features/platform/webhooks/WebhooksView';
...
export default async function WebhooksPage({ params }: { params: Promise<{ workspace: string }> }) {
  const [{ workspace: slug }, t] = await Promise.all([
    params,
    getTranslations('platform.webhooks'),
  ]);
  ...
  return (
    <section className="flex flex-col gap-6">
      <WebhooksView workspaceSlug={slug} canManage />
    </section>
  );
}
