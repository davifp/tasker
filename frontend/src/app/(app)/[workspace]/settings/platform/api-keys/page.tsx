...
import { ApiKeysView } from '@/features/platform/api-keys/ApiKeysView';
...
export default async function ApiKeysPage({ params }: { params: Promise<{ workspace: string }> }) {
  const [{ workspace: slug }, t] = await Promise.all([params, getTranslations('platform.apiKeys')]);
  ...
  return (
    <section className="flex flex-col gap-6">
      <ApiKeysView workspaceSlug={slug} canManage />
    </section>
  );
}
