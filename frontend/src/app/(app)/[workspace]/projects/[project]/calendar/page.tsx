...
// identifiers off to the client `<CalendarView />`. Mirrors the List /
// Backlog shells so the project header (rendered by `layout.tsx`), the
// filter chip bar, and the tab body share one visual rhythm.
...
// TODO(workspaces-tz): source the workspace timezone from
// `Workspace.timezone` (backend model) once the field lands. Until
// then default to `'UTC'` so bucketing is deterministic across
// developer machines and CI.
const DEFAULT_WORKSPACE_TZ = 'UTC';

export default async function ProjectCalendarPage({ params }: CalendarPageProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <CalendarView
        ...
        workspaceTimezone={DEFAULT_WORKSPACE_TZ}
      />
    </div>
  );
}
