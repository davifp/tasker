...
@Controller('workspaces/:slug/integrations/github')
export class GithubIntegrationController {
  ...
  async start(
    ...
    const state = await this.state.issue('GITHUB', ctx.workspaceId, ctx.userId, dto.returnTo);
    const authorizeUrl = this.github.buildAuthorizeUrl(state, this.redirectUri());
    return {
      authorizeUrl,
      scopes: this.github.scopesForConsentCopy(),
    };
  }
  ...
  @Auditable({
    ...
    targetIdFrom: (_req, body) =>
      (body as { integrationId?: string } | null)?.integrationId ?? 'GITHUB',
  })
  async complete(
    ...
    const ctx = requireCtx(req);
    const stored = await this.state.consume(dto.state, 'GITHUB');
    ...
    return this.github.completeConnection({
      code: dto.code,
  ...
  private redirectUri(): string {
    ...
    return `${base}/api/v1/integrations/github/callback`;
  }
}
